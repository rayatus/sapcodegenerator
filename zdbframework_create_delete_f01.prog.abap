...
*----------------------------------------------------------------------*
*&---------------------------------------------------------------------*
*&      Form  CREATE_DELETE_DETAIL
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
FORM create_delete_detail  USING   id_clsname           TYPE seoclsname
...
* Creamos el método
  ...
  <ls_method_sources>-cpdname = <ls_method>-cmpname   = 'DELETE_DETAILS'.
  <ls_method>-descript   = 'Delete Details'.
...
* Creamos el parámetro tabla de entrada
  ...
  <ls_parameter>-sconame    = 'IS_DETAILS'.
...
*  <ls_exception>-version  = seoc_version_active.
  <ls_exception>-descript = 'Error while deleting'.

* Definimos variables locales al método
  ...
  mac_add_source '  DATA lo_exception TYPE REF TO  cx_c2s_db.'.

* Procedemos a BORRAR en BBDD (creando o modificando el registro por claves)
  mac_add_source space.
  mac_add_source '  CHECK is_details IS NOT INITIAL.'.
  mac_add_source '  TRY .'.
  mac_add_source '      MOVE-CORRESPONDING is_details TO ls_list.'.
  mac_add_source '      INSERT ls_list INTO TABLE lt_list[].'.
  mac_add_source '      delete_list( lt_list[] ).'.
  mac_add_source '    CATCH cx_c2s_db INTO lo_exception.'.
  mac_add_source '      RAISE EXCEPTION lo_exception.'.
  mac_add_source '  ENDTRY.'.
ENDFORM.                    " CREATE_DELETE_DETAIL
